...
export function useFleet() {
  ...
  const fleet = useMemo<FleetDriver[]>(() => Object.values(locations).map((location) => ({ ...location, connection: Date.now() - new Date(location.receivedAt).getTime() < 30000 ? 'online' : 'stale' })), [locations]);
