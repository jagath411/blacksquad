...
class LocationStateService {
  ...
  public async update(input: Omit<DriverLocation, 'receivedAt' | 'sequence'>): Promise<LocationUpdateResult> {
    ...
    try {
      ...
      const userDoc = driver.userId as any;
      ...
      const location: DriverLocation = { ...input, driverName, sequence, receivedAt: new Date().toISOString() };
  ...
  public async loadFromDatabase(): Promise<void> {
    ...
    for (const row of latest as Array<{ _id: { toString(): string }; location: { location: { coordinates: [number, number] }; speed?: number; heading?: number; accuracy?: number; timestamp: Date; createdAt: Date } }>) {
      ...
      const driver = await DriverModel.findById(driverObjId).populate('userId', 'name email').lean();
      const userDoc = driver?.userId as any;
      ...
      this.locations.set(driverId, { driverId, driverName, latitude: coords[1], longitude: coords[0], speed: row.location.speed, heading: row.location.heading, accuracy: row.location.accuracy, timestamp: row.location.timestamp.getTime(), receivedAt: row.location.createdAt.toISOString(), sequence: 0 });
    }
  }
