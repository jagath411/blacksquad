...
router.post(
  ...
  async (req: AuthenticatedRequest, res, next) => {
    try {
      ...
      if (!result.accepted) {
        res.status(result.reason === 'driver_not_found' ? 409 : 202).json({
          success: false,
          accepted: false,
          message:
            result.reason === 'driver_not_found'
              ? 'Driver profile is not provisioned'
              : 'Stale location ignored',
        });
        return;
      }
...
router.get('/', requireAuth, requireRole('OWNER'), (_req, res) =>
  res.json({ success: true, locations: locationState.all() }),
);
